...
- Optional debugging parameter: `--preserve-on-failure`.
- Optional debugging parameter: `--start-at <entry-number>`.
...
- If `--preserve-on-failure` is not provided, failure paths run registered
  cleanup blocks and automatic patch restores before returning.
- If `--start-at` is not provided, execution starts at the first runbook entry.
- If `--start-at` is provided, it must be a 1-based entry number within the
  runbook's `entries` array.
...
- If `sw` is invoked without a subcommand, `sw --debug` behaves the same as
  `sw run --debug`.
- If `sw` is invoked without a subcommand, `sw --preserve-on-failure` behaves
  the same as `sw run --preserve-on-failure`.
- If `sw` is invoked without a subcommand, `sw --start-at <entry-number>`
  behaves the same as `sw run --start-at <entry-number>`.
...
- `sw run --preserve-on-failure` preserves failure state for debugging long
  runbooks.
- `--preserve-on-failure` only changes behavior when the run stops because of
  a command failure, timeout, patch application failure, prerequisite failure,
  or operational execution failure.
- On failure with `--preserve-on-failure`, registered explicit command cleanup
  blocks are not executed.
- On failure with `--preserve-on-failure`, automatic patch restoration is not
  executed for patches already applied during that run.
- On failure with `--preserve-on-failure`, timeout-driven termination of the
  active command process group still applies. The flag preserves the workspace
  state after the failed entry; it does not preserve a hung process tree by
  default.
- On success with `--preserve-on-failure`, normal cleanup and automatic patch
  restoration still run.
- On `Breakpoint` with `--preserve-on-failure`, normal cleanup and automatic
  patch restoration still run because a breakpoint is an intentional successful
  stop, not a failure.
- When cleanup or patch restoration is skipped because of
  `--preserve-on-failure`, the failure message reports that run state was
  preserved.
- `sw run --start-at <entry-number>` starts rendering and execution at a
  1-based runbook entry number.
- `--start-at` uses the same entry numbering shown by verbose progress output,
  so verbose entry `[113/138]` corresponds to `--start-at 113`.
- `--start-at` skips rendering and execution for entries before the selected
  entry.
- `--start-at` still validates the whole runbook before execution begins.
- `--start-at` runs prerequisite checks only for prerequisite entries at or
  after the selected entry and before the first breakpoint in that selected
  range.
- Verbose progress for `--start-at` preserves original entry numbers and the
  original total entry count.
- Output generated by a `--start-at` run is partial debug output that begins at
  the selected entry. It is not intended to regenerate a complete README.
- `--preserve-on-failure` and `--start-at` may be used together.
- When `sw run` fails at a concrete runbook entry, stderr includes a final
  resume hint in the form
  `Run: sw run <original-run-options> --start-at <entry-number> to resume from this entry`.
- The resume hint uses the failed entry's 1-based runbook entry number.
- The resume hint preserves applicable run options such as `--verbose`,
  `--debug`, `--input-file`, `--input-format`, `--working-directory`,
  `--output-format`, `--output-file`, and `--preserve-on-failure`.
- The resume hint uses `--start-at`, matching the implemented CLI flag.
- The resume hint is not emitted for validation failures or operational
  failures that happen before a concrete runbook entry begins.
...
- A `Breakpoint` entry intentionally stops `sw run` at that point with exit
  code `0`.
- Entries before the `Breakpoint` render and execute normally.
- The `Breakpoint` entry renders a short Markdown note so the generated output
  shows where execution was intentionally stopped.
- If `Breakpoint.message` is present, it is included in that rendered note.
- Entries after the `Breakpoint` are not rendered or executed.
- Deferred Markdown interpolation for output generated before a `Breakpoint`
  preserves unresolved placeholders that would only have been captured by
  skipped later entries.
- Cleanup blocks and patch restores registered before the `Breakpoint` still
  run after processing stops.
...
- Each prerequisite check may declare `help` as a human-readable remediation
  message.
- Prerequisite checks execute before normal runbook command entries, but only
  for `Prerequisite` entries that appear before the first `Breakpoint`.
...
- `Breakpoint` entries are intended for debugging and incremental runbook
  authoring.
- A `Breakpoint` entry may declare `message`.
- If `message` is present, it must be a string.
- `Breakpoint` does not execute shell commands.
- `Breakpoint` stops `sw run` successfully rather than treating the stop as a
  failure.
- `Breakpoint` does not stop `sw validate`; validation still checks the whole
  runbook.
- `Breakpoint` stops `sw check` from evaluating prerequisite entries declared
  after the breakpoint.
- `Breakpoint` supports no fields other than `type` and `message`.
