...
- [`sw init`](./docs/guides/init.md) generates a starter runbook file in YAML by
  default.
- [`sw import`](./docs/guides/import.md) turns an existing `README.md` into a
  starter runbook.
- [`sw validate`](./docs/guides/validate.md) checks that a runbook is
  structurally valid without executing it.
- [`sw check`](./docs/guides/check.md) validates the runbook and runs only
  prerequisite checks.
- [`sw run`](./docs/guides/run.md) executes the runbook and writes generated
  Markdown output.
- [`sw format`](./docs/guides/format.md) rewrites a runbook file in place using
  the canonical formatting for its current format.
- [`sw convert`](./docs/guides/convert.md) converts a runbook file between YAML
  and JSON.
- [`sw example`](./docs/guides/example.md) prints focused runbook snippets for
  supported topics.
- [`sw explain`](./docs/guides/explain.md) describes feature contracts,
  defaults, and discovery paths.
- [`sw help`](./docs/guides/help.md) shows command-line syntax and subcommand
  help.
- [`sw version`](./docs/guides/version.md) prints the current build identity.
...
The generated `sw` skill is a compact operating guide. It gives agents the
first workflow rules they need, then routes detailed behavior questions back
to `sw explain`, `sw help`, and `sw example` so command knowledge is not
...
The repository pins Rust `1.95.0` through `rust-toolchain.toml` so local
