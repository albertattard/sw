...
fn run_help_prints_help() {
    ...
    assert!(stdout.contains(
        "`DisplayFile.content_type` and `DisplayUrl.content_type` override fence detection; otherwise `.java`, `.md`, `.markdown`, `.sh`, `.sql`, `.xml`, `Dockerfile`, and `Dockerfile-*` are detected as lowercase `dockerfile`"
    ));
    assert!(
