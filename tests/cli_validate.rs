...
#[test]
fn validate_accepts_scalar_command_scripts_in_yaml() {
    let output = run(&[
        "validate",
        "--input-file",
        "tests/fixtures/sw-runbook-scalar-command.yaml",
        "--output-format",
        "json",
    ]);
...
fn validate_accepts_json_runbook_from_stdin() {
    ...
    let output = run_in_dir_with_stdin(
        &["validate", "--input-file=-", "--output-format=json"],
        &dir,
        &stdin,
    );
...
#[test]
fn validate_accepts_yaml_runbook_from_stdin_with_explicit_format() {
    let dir = prepare_workspace();
...
#[test]
fn validate_rejects_yaml_runbook_from_stdin_without_explicit_format() {
    let dir = prepare_workspace();
    let stdin = fs::read_to_string("tests/fixtures/sw-runbook-run-success.yaml")
        .expect("failed to read fixture");
    ...
    assert!(stdout.contains("Invalid JSON in stdin"));
}
...
fn validate_input_format_without_stdin_keeps_default_file_lookup() {
    ...
    let output = run_in_dir(
        &["validate", "--input-format=yaml", "--output-format=json"],
        &dir,
    );
...
#[test]
fn invalid_output_trim_empty_lines_returns_validation_failure() {
    let output = run(&[
        "validate",
        "--input-file",
        "tests/fixtures/sw-runbook-invalid-output-trim-empty-lines.json",
        "--output-format",
        "json",
    ]);
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("\"valid\": false"));
    assert!(stdout.contains("\"path\": \"entries[0].output.trim_empty_lines\""));
    assert!(stdout.contains("must be one of `leading_trailing`, `leading`, `trailing`, or `none`"));
}
