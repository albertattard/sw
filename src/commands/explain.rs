...
fn build_skill_document() -> String {
    let lines = vec![
        "---".to_string(),
        "name: sw".to_string(),
        "description: Use this skill when the user needs help authoring or executing Sociable Weaver (`sw`) tested documentation runbooks.".to_string(),
        "---".to_string(),
        "".to_string(),
        "# sw".to_string(),
        "".to_string(),
        "`sw` is an authoring and execution tool for tested, executable documentation runbooks. It checks prerequisites, runs documented workflows, and incorporates real command output into generated Markdown documentation.".to_string(),
        "".to_string(),
        "## Discovery".to_string(),
        "".to_string(),
        "Choose the narrowest authoritative command for the task:".to_string(),
        "".to_string(),
        "```shell".to_string(),
        "sw explain <topic>      # behaviour and defaults".to_string(),
        "sw help <command>       # exact invocation syntax".to_string(),
        "sw example <EntryType>  # unfamiliar runbook entry shapes".to_string(),
        "```".to_string(),
        "".to_string(),
        "Use `sw explain --all` only when you need broad discovery of the current command contract. Treat `sw` output and the repository specs as authoritative; do not rely on cached assumptions about runbook fields, defaults, or supported subcommands.".to_string(),
        "".to_string(),
        "## Further Reference".to_string(),
        "".to_string(),
        "For an overview of runbook entry types and their relationships, see the [runbook entry guide](https://github.com/albertattard/sw/blob/main/docs/guides/entities.md). Prefer `sw explain` and `sw example` when you need the contract for the currently installed binary.".to_string(),
        "".to_string(),
        "## Common Workflows".to_string(),
        "".to_string(),
        "- Use `sw init` to create a starter YAML runbook.".to_string(),
        "- Use `sw example <EntryType>` to get a current YAML snippet for entries such as `Breakpoint`, `ChangeDirectory`, `Command`, `DisplayFile`, `DisplayUrl`, `Patch`, or `Prerequisite`.".to_string(),
        ...
        "- Use `sw run --input-file <runbook>` to execute the runbook and generate Markdown output. It writes `./README.md` by default; pass `--output-file` when that target is not intended.".to_string(),
        "- Use `sw format --input-file <runbook>` to rewrite a runbook in place using canonical formatting.".to_string(),
        "- Use `sw convert --input-file <runbook>` to convert between JSON and YAML.".to_string(),
        "".to_string(),
        "## Authoring Defaults".to_string(),
        "".to_string(),
        ...
        "- If more than one default runbook file exists, pass `--input-file` explicitly.".to_string(),
        "".to_string(),
        "## Agent Rules".to_string(),
        "".to_string(),
        "- Before inventing a field, run `sw explain <topic>` or `sw example <EntryType>`.".to_string(),
        "- Before telling the user a runbook is valid, run `sw validate`.".to_string(),
        "- Treat `sw run` as command execution. If the user explicitly asks to run a runbook, proceed within that request's scope. Otherwise, inspect the runbook and ask for confirmation before running it.".to_string(),
        "- Do not edit generated `README.md` directly when `sw-runbook.yaml` is the source.".to_string(),
    ];
    ...
    vec![
        ...
        Explanation {
            ...
            constraints: &[
                ...
                "`--input-file` remains relative to the shell current directory and is not resolved from `--working-directory`.",
        ...
        Explanation {
            ...
            inputs: &[
                ...
                "`--input-format json|yaml` for stdin-backed input",
                "`sw --output-format markdown` or `sw run --output-format markdown`",
                "`sw --output-file <path>` or `sw run --output-file <path>`",
                "`sw --working-directory <path>` or `sw run --working-directory <path>`",
            ...
            constraints: &[
                ...
                "`--working-directory` must already exist and must be a directory.",
                "`--input-file` and `--output-file` remain normal CLI paths and are not resolved from `--working-directory`.",
                "When `--working-directory` is omitted, the execution root is the runbook file's directory, or the shell current directory for stdin-backed runbooks.",
                "`Markdown.contents`, `Command.commands`, `Patch.patch`, `Prerequisite.checks[*].contents`, and `Prerequisite.checks[*].commands` may be a single string or an array of strings.",
                ...
                "`ChangeDirectory.path` establishes an active directory for later normal Command entries without rendering a shell `cd`; optional `contents` renders explanatory Markdown followed by a visible working-directory note.",
                "ChangeDirectory affects normal commands only. It does not affect prerequisite checks, display entries, or patches; an explicit Command working_directory remains execution-root-relative.",
                ...
                "`DisplayFile.content_type` and `DisplayUrl.content_type` override fence detection and accept `properties`; otherwise `.java`, `.md`, `.markdown`, `.sh`, `.sql`, `.xml`, `.yaml`, `.yml`, `Dockerfile`, and `Dockerfile-*` are detected as lowercase `dockerfile`, with other extensions falling back to `text`.",
        ...
        Explanation {
            ...
            inputs: &[
                ...
                "Supported topics currently include `Breakpoint`, `ChangeDirectory`, `Command`, `DisplayFile`, `DisplayUrl`, `Patch`, and `Prerequisite`.",
            ],
            outputs: &[
            ...
            constraints: &[
                ...
                "The `ChangeDirectory` example shows an optional explanation and persistent command context.",
            ...
            next: &[
                ...
                "Use `sw example DisplayUrl` when you need to render a remote raw text URL.",
