...
pub struct Cli {
    ...
    #[command(flatten)]
    pub default_run_debug: RunDebugArgs,
...
#[derive(Debug, Default, Clone, clap::Args)]
pub struct RunDebugArgs {
    /// Preserve registered cleanup and patch restore state when a run fails.
    #[arg(long, global = true)]
    pub preserve_on_failure: bool,

    /// Start at a 1-based runbook entry number for a partial debug run.
    #[arg(long, global = true, value_name = "ENTRY_NUMBER")]
...
pub struct ExampleArgs {
    /// Example topic such as `Breakpoint`, `Command`, `DisplayFile`, `DisplayUrl`, or `Prerequisite`.
    pub topic: String,
...
impl RunDebugArgs {
    pub fn has_values(&self) -> bool {
        self.preserve_on_failure || self.start_at.is_some()
    }
}
